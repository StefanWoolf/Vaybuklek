...
_PRIORITY_UP = (
    ...
    "важн", "срочн", "asap", "горит", "критич", "немедленно", "сегодня же", "повыше",
)
_PRIORITY_DOWN = (
...
def _match_assignees(text: str, ctx: ExtractionContext) -> tuple[list[str], str]:
    """Вернуть (список исполнителей, текст без префикса имени).

    Ловит несколько исполнителей: «Маша и Паша», «Маша, Паша, Витя».
    """
    primary, rest = _match_assignee(text, ctx)
    found: list[str] = []

    def _add(name: str) -> None:
        bare = name.lstrip("@").strip()
        if bare and not any(bare.lower() == f.lstrip("@").lower() for f in found):
            found.append(name)
...
_RU_VOWEL_END = "аяеёийоуыэюь"


def find_team_mentions(text: str, ctx: ExtractionContext) -> list[tuple[str, str]]:
    """Найти упоминания участников с учётом простых склонений.

    «назначь на Дашу» → участник «Даша». Возвращает пары
    (каноническое_имя, найденная_словоформа) — словоформа нужна для вырезания.
    """
    low = text.lower()
    ...
    for name in ctx.team_names():
        bare = name.lstrip("@")
        root = bare
        if root and root[-1].lower() in _RU_VOWEL_END:
            root = root[:-1]
        if len(root) < 3:
            root = bare
        m = re.search(rf"\b({re.escape(root)}[а-яёa-z]*)\b", low, re.IGNORECASE)
        if m and name.lower() not in seen:
            seen.add(name.lower())
...
_ASSIGN_DIRECTIVE = re.compile(
    r"\b(назнач\w*|повес\w*|переназнач\w*|поручи\w*|исполнител\w*|добав\w*|"
    r"подключ\w*|вместе\s+с|также|тоже|ещё|еще|в\s+помощь)\b\s*(?:на\s+)?",
    re.IGNORECASE,
)
...
def _strip_field_directives(text: str, assignees_raw: str | list[str] | None) -> str:
    ...
    for a in assignees_raw or []:
        t = re.sub(re.escape(a.lstrip("@")) + r"\w*", " ", t, flags=re.IGNORECASE)
    t = _DEADLINE_DIRECTIVE.sub(" ", t)
    t = re.sub(r"\bна\b", " ", t)  # остаточный предлог после «назначь на»/«перенеси на»
    return re.sub(r"\s{2,}", " ", t).strip(" \t\n.,—-")


def correction_is_reformulation(correction: str, assignees_raw: str | list[str] | None) -> bool:
    ...
    """
    residual = _strip_field_directives(correction, assignees_raw)
    ...
    async def _extract_from(
        ...
        for clause in clauses:
            ...
            if len(clause) < 4 or not _looks_like_task(clause):
                continue
            assignees, rest = _match_assignees(clause, context)
